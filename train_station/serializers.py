...
class CrewDetailSerializer(CrewSerializer):
    journeys = serializers.SlugRelatedField(
        many=True, read_only=True, slug_field="info"
    )
...
class JourneyListSerializer(JourneySerializer):
    route = serializers.SlugRelatedField(
        many=False, read_only=True, slug_field="route"
    )
...
class JourneyDetailSerializer(JourneySerializer):
    source = StationSerializer(
        many=False, read_only=True, source="route.source"
    )
    destination = StationSerializer(
        many=False, read_only=True, source="route.destination"
    )
    distance = serializers.SlugRelatedField(
        many=False, read_only=True, source="route", slug_field="distance"
    )
